...
from keras.layers import Activation, Conv1D, Dense, Dropout, Input, concatenate
...
def model(shape, window, st, nt):
    ...
    mdl = Model(
        inputs=in1,
        outputs=output,
        name="{}_{}_{}nt_base_mdl_crossval".format(st, nt, str(window)),
    )
...
def plot(history, window, st, nt):
    ...
    # Set title
    plt.title(
        "{} {} {}nt Fold Training and Testing Accuracy by Epoch".format(
            st, nt, str(window)
        ),
        fontsize=25,
    )
    ...
    # Set title
    plt.title(
        "{} {} {}nt Base Training and Testing Loss by Epoch".format(
            st, nt, str(window)
        ),
        fontsize=25,
    )
...
def aucroc(mdl, x_test, y_test, window, st, nt):
    ...
    np.save(
        "plots_arrays/roc/{}_{}_{}nt_base_fpr".format(st, nt, str(window)), fpr_keras
    )
    np.save(
        "plots_arrays/roc/{}_{}_{}nt_base_tpr".format(st, nt, str(window)), tpr_keras
    )
...
def aupr(mdl, x_test, y_test, window, st, nt):
    ...
    np.save(
        "plots_arrays/pr/{}_{}_{}nt_base_precision".format(st, nt, str(window)),
        precision,
    )
    np.save(
        "plots_arrays/pr/{}_{}_{}nt_base_recall".format(st, nt, str(window)), recall
    )
...
def main():
    ...
    if not args["positive"]:
        ...
        pred_loc = "predictions/{}_{}_{}nt_predictions.csv".format(
        ...
        model_loc = "models/{}_{}_{}nt_base_model".format(
            sample_type, negative_type, str(window)
        )
        mdl.save(model_loc)
