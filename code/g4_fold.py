...
from keras.layers import Activation, Conv1D, Dense, Dropout, Input
...
def check_dir():
    ...
    dir_list = [
        "g4d_out" "g4d_out/plots",
        "g4d_out/models",
        "g4d_out/predictions",
        "g4d_out/plot_arrays",
        "g4d_out/plot_arrays/roc",
        "g4d_out/plot_arrays/pr",
    ]
    for d in dir_list:
...
def model(shape, window, st, nt):
    ...
    mdl = Model(
        inputs=in1,
        outputs=output,
        name="{}_{}_{}nt_fold_mdl_crossval".format(st, nt, str(window)),
    )
...
def plot(history, window, st, nt):
    ...
    # Set title
    plt.title(
        "{} {} {}nt Fold Training and Testing AUC by Epoch".format(st, nt, str(window)),
        fontsize=25,
    )
...
def aucroc(mdl, x_test, y_test, window, st, nt):
    ...
    np.save(
        "plots_arrays/roc/{}_{}_{}nt_fold_fpr".format(st, nt, str(window)), fpr_keras
    )
    np.save(
        "plots_arrays/roc/{}_{}_{}nt_fold_tpr".format(st, nt, str(window)), tpr_keras
    )
...
def aupr(mdl, x_test, y_test, window, st, nt):
    ...
    np.save(
        "plots_arrays/pr/{}_{}_{}nt_fold_precision".format(st, nt, str(window)),
        precision,
    )
    np.save(
        "plots_arrays/pr/{}_{}_{}nt_fold_recall".format(st, nt, str(window)), recall
    )
...
def user_input():
    ...
    parser.add_argument(
        "-p",
        "--positive",
        help="Path to .fa positive examples.",
        type=str,
        required=True,
    )
    ...
    parser.add_argument(
        "-n",
        "--negative",
        help="Path to .fa negative examples.",
        type=str,
        required=True,
    )
    ...
    parser.add_argument(
        "-fp",
        "--fold_positive",
        help="Path to directory of positive examples RNAplfold lunp files.",
        type=str,
        required=False,
    )
    ...
    parser.add_argument(
        "-fn",
        "--fold_negative",
        help="Path to directory of negative examples RNAplfold lunp files.",
        type=str,
        required=False,
    )
...
def main():
    ...
    if not args["positive"]:
        ...
        pred_loc = "predictions/{}_{}_{}nt_fold_predictions.csv".format(
            sample_type, negative_type, str(window)
        )

        df.to_csv(pred_loc)
        print("Lables File at {}".format(pred_loc))

        model_loc = "models/{}_{}_{}nt_fold_model".format(
            sample_type, negative_type, str(window)
        )
        mdl.save(model_loc)
